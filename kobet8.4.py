...
class Suchroom :
    ...
    def Beting(self,A,E,B,C,D,F):#다운ex : from1to18, from1to18, ROU_118, 46, 316,
        self.TTa = False
        while TTa == False:
            if x == "7x0b1tgh7agmf6hv":#이머징 룰렛
                wss.send(json.dumps({"log":{"type":"CLIENT_BET_CHIP","value":{"amount":F,"type":"bet_%s","chips":"%s@bet_%s","codes":{"%s":F},"balance":9000,"viewType":"Immersive (Overlay, Full screen, Multi-camera View)","gameType":"roulette","appVersion":4,"maxTouchPoints":0,"currency":"KRW","chipStack":[1000,2000,10000,20000,1000000,2000000],"defaultChip":1,"tableMinLimit":F,"tableMaxLimit":3000000,"channel":"PCMac","orientation":"landscape"}}}))%(A,E,B,C)
                wss.send(json.dumps({"args":{"gameId":self.gameIdS(),"action":{"type":"PLACE","value":{"%s":F}},"timestamp":self.tt(),"betTags":{"mwLayout":8,"openMwTables":1,"latency":random.random(-100,400),"videoProtocol":"flvjs","btTableView":"hd1","btVideoQuality":"immersive_hi","btMiniGame":0,"appVersion":4,"orientation":"landscape"}},"id":self.wid(),"type":"roulette.betAction"}))%(D,E)
                ...
                if data1["type"] == "roulette.betActionResponse":
                    TTa = True
            elif x == "LightningTable01":#라이트링 룰렛 48
                wss.send(json.dumps({"log":{"type":"CLIENT_BET_CHIP","value":{"amount":F,"type":"bet_%s","chips":"%s@bet_red","codes":{"%s":F},"balance":9640,"viewType":"Immersive (Overlay, Full screen, Multi-camera View)","gameType":"roulette","appVersion":4,"maxTouchPoints":0,"currency":"KRW","chipStack":[200,1000,4000,20000,100000,400000],"defaultChip":1,"tableMinLimit":F,"tableMaxLimit":2000000,"channel":"PCMac","orientation":"landscape"}}}))%(A,E,B,C)
                wss.send(json.dumps({"args":{"gameId":self.gameIdS(),"action":{"type":"PLACE","value":{"%s":F}},"timestamp":self.tt(),"betTags":{"mwLayout":8,"openMwTables":1,"latency":random.random(-100,400),"videoProtocol":"flvjs","btTableView":"hd1","btVideoQuality":"lightr1_imr_med","btMiniGame":0,"appVersion":4,"orientation":"landscape"}},"id":self.wid(),"type":"roulette.betAction"}))%(D,E)
                wss.send(json.dumps({"log":{"type":"CLIENT_EXITED_RFB_EDIT_MODE","value":{"channel":"PCMac","orientation":"landscape"}}}))
                result =  wss.recv()
                data1 = json.loads(result)
                if data1["type"] == "roulette.betActionResponse":
