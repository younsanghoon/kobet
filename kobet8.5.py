...
class Suchroom :
    ...
    def Beting(self,A,B,C,D):#블랙ex : 
        self.TTa = False
        while self.TTa == False:
            if self.x == "LightningTable01":#라이트링 룰렛 48 A = 200 C = 200 B= bet_black D = ROU_Black  E = 남음돈
                wss.send(json.dumps({"log":{"type":"CLIENT_BET_CHIP","value":{"amount":A,"type":"%s","chips":"%s@%s","codes":{"%s":A},"balance":E,"viewType":
                                                                              "Immersive (Overlay, Full screen, Multi-camera View)","gameType":"roulette","appVersion":4,"maxTouchPoints"
                                                                              :0,"currency":"KRW","chipStack":[200,1000,4000,20000,100000,400000],"defaultChip":1,"tableMinLimit":200,
                                                                              "tableMaxLimit":2000000,"channel":"PCMac","orientation":"landscape"}}}))%(B,C,B,D)
                
                wss.send(json.dumps({"args":{"gameId":self.gameIdS(),"action":{"type":"PLACE","value":{"49":A}},"timestamp":self.tt(),"betTags":{"mwLayout":8,"openMwTables":1,"latency":random.random(-200,500),"videoProtocol":"flvjs","btTableView":"hd1","btVideoQuality":"lightr1_imr_hi","btMiniGame":0,"appVersion":4,"orientation":"landscape"}},"id":self.wid(),"type":"roulette.betAction"}))
                ...
                if data1["type"] == "roulette.betActionResponse":
                    self.TTa = True
            elif self.x == "7x0b1tgh7agmf6hv":#이머징 룰렛
                wss.send(json.dumps({"log":{"type":"CLIENT_BET_CHIP","value":{"amount":A,"type":"bet_black","chips":"1000@bet_black","codes":{"ROU_Black":A},"balance":8640,"viewType":"Immersive (Overlay, Full screen, Multi-camera View)","gameType":"roulette","appVersion":4,"maxTouchPoints":0,"currency":"KRW","chipStack":[1000,2000,10000,20000,1000000,2000000],"defaultChip":1,"tableMinLimit":1000,"tableMaxLimit":3000000,"channel":"PCMac","orientation":"landscape"}}}))
                wss.send(json.dumps({"args":{"gameId":"15e343dff2eae284003975b7","action":{"type":"PLACE","value":{"49":A}},"timestamp":self.tt(),"betTags":{"mwLayout":8,"openMwTables":1,"latency":293,"videoProtocol":"flvjs","btTableView":"hd1","btVideoQuality":"immersive_hi","btMiniGame":0,"appVersion":4,"orientation":"landscape"}},"id":"wvcmxia53w","type":"roulette.betAction"}))
                wss.send(json.dumps({"log":{"type":"CLIENT_EXITED_RFB_EDIT_MODE","value":{"channel":"PCMac","orientation":"landscape"}}}))
                result =  wss.recv()
                data1 = json.loads(result)
                if data1["type"] == "roulette.betActionResponse":
                    self.TTa = True
            elif self.x == "vctlz20yfnmp1ylr": #룰렛
                wss.send(json.dumps({"log":{"type":"CLIENT_BET_CHIP","value":{"amount":A,"type":"bet_black","chips":"400@bet_black","codes":{"ROU_Black":A},"balance":9240,"viewType":"Immersive (Overlay, Full screen, Multi-camera View)","gameType":"roulette","appVersion":4,"maxTouchPoints":0,"currency":"KRW","chipStack":[400,800,2000,10000,20000,1000000],"defaultChip":1,"tableMinLimit":400,"tableMaxLimit":3000000,"channel":"PCMac","orientation":"landscape"}}}))
                wss.send(json.dumps({"args":{"gameId":"15e343fe79bdfc5ab47eab64","action":{"type":"PLACE","value":{"49":A}},"timestamp":self.tt(),"betTags":{"mwLayout":8,"openMwTables":1,"latency":295,"videoProtocol":"flvjs","btTableView":"hd1","btVideoQuality":"green_imr_hi","btMiniGame":0,"appVersion":4,"orientation":"landscape"}},"id":"fcr64uewca","type":"roulette.betAction"}))
                wss.send(json.dumps({"log":{"type":"CLIENT_EXITED_RFB_EDIT_MODE","value":{"channel":"PCMac","orientation":"landscape"}}}))
                result =  wss.recv()
                data1 = json.loads(result)
                if data1["type"] == "roulette.betActionResponse":
                    self.TTa = True
            elif self.x == "r5aw9yumyaxgnd90" :#터키 룰렛
                wss.send(json.dumps({"log":{"type":"CLIENT_BET_CHIP","value":{"amount":A,"type":"bet_black","chips":"1000@bet_black","codes":{"ROU_Black":A},"balance":8640,"viewType":"Immersive (Overlay, Full screen, Multi-camera View)","gameType":"roulette","appVersion":4,"maxTouchPoints":0,"currency":"KRW","chipStack":[1000,2000,5000,25000,100000,500000],"defaultChip":1,"tableMinLimit":1000,"tableMaxLimit":2000000,"channel":"PCMac","orientation":"landscape"}}}))
                wss.send(json.dumps({"args":{"gameId":"15e3441f892ff662435d7e1c","action":{"type":"PLACE","value":{"49":A}},"timestamp":self.tt(),"betTags":{"mwLayout":8,"openMwTables":1,"latency":280,"videoProtocol":"flvjs","btTableView":"hd1","btVideoQuality":"tk2_imr_hi","btMiniGame":0,"appVersion":4,"orientation":"landscape"}},"id":"msaxvgce8f","type":"roulette.betAction"}))
                wss.send(json.dumps({"log":{"type":"CLIENT_EXITED_RFB_EDIT_MODE","value":{"channel":"PCMac","orientation":"landscape"}}}))
                result =  wss.recv()
                data1 = json.loads(result)
                if data1["type"] == "roulette.betActionResponse":
                    self.TTa = True
            elif self.x == "zosmk25g2f768o52" : #룰렛 라이브
                wss.send(json.dumps({"log":{"type":"CLIENT_BET_CHIP","value":{"amount":A,"type":"bet_black","chips":"800@bet_black","codes":{"ROU_Black":A},"balance":8840,"viewType":"Immersive (Overlay, Full screen, Multi-camera View)","gameType":"roulette","appVersion":4,"maxTouchPoints":0,"currency":"KRW","chipStack":[800,2000,10000,20000,1000000,2000000],"defaultChip":1,"tableMinLimit":800,"tableMaxLimit":3000000,"channel":"PCMac","orientation":"landscape"}}}))
                wss.send(json.dumps({"args":{"gameId":"15e34443bbbd65eb51340c97","action":{"type":"PLACE","value":{"49":A}},"timestamp":self.tt(),"betTags":{"mwLayout":8,"openMwTables":1,"latency":-4711,"videoProtocol":"flvjs","btTableView":"hd1","btVideoQuality":"rugent1_imr_hi","btMiniGame":0,"appVersion":4,"orientation":"landscape"}},"id":"g7aym2bqj0","type":"roulette.betAction"}))
                wss.send(json.dumps({"log":{"type":"CLIENT_EXITED_RFB_EDIT_MODE","value":{"channel":"PCMac","orientation":"landscape"}}}))
                result =  wss.recv()
                data1 = json.loads(result)
                if data1["type"] == "roulette.betActionResponse":
                    self.TTa = True
            elif self.x == "48z5pjps3ntvqc1b" : #오토 룰렛
                wss.send(json.dumps({"log":{"type":"CLIENT_BET_CHIP","value":{"amount":160,"type":"bet_black","chips":"160@bet_black","codes":{"ROU_Black":160},"balance":9480,"viewType":"Slingshot","gameType":"roulette","appVersion":4,"maxTouchPoints":0,"currency":"KRW","chipStack":[160,1000,4000,20000,100000,400000],"defaultChip":1,"tableMinLimit":160,"tableMaxLimit":2000000,"channel":"PCMac","orientation":"landscape"}}}))
                wss.send(json.dumps({"args":{"gameId":"15e3447dee8512368aca1850","action":{"type":"PLACE","value":{"49":160}},"timestamp":self.tt(),"betTags":{"mwLayout":8,"openMwTables":1,"latency":271,"videoProtocol":"flvjs","btTableView":"slingshot","btVideoQuality":"gen1_ss_hi","btMiniGame":0,"appVersion":4,"orientation":"landscape"}},"id":"2nn1yupgdz","type":"roulette.betAction"}))
                wss.send(json.dumps({"id":"0pv1mbtdjl","type":"metrics.ping","args":{"t":1577179607365}}))
                result =  wss.recv()
                data1 = json.loads(result)
                if data1["type"] == "roulette.betActionResponse":
                    self.TTa = True
            elif self.x == "01rb77cq1gtenhmo" : #vip 오토 룰렛
                wss.send(json.dumps({"log":{"type":"CLIENT_BET_CHIP","value":{"amount":300,"type":"bet_black","chips":"300@bet_black","codes":{"ROU_Black":300},"balance":9340,"viewType":"Slingshot","gameType":"roulette","appVersion":4,"maxTouchPoints":0,"currency":"KRW","chipStack":[300,1000,2000,10000,20000,1000000],"defaultChip":1,"tableMinLimit":300,"tableMaxLimit":3000000,"channel":"PCMac","orientation":"landscape"}}}))
                wss.send(json.dumps({"args":{"gameId":"15e3449d6521898f4c9c2038","action":{"type":"PLACE","value":{"49":300}},"timestamp":self.tt(),"betTags":{"mwLayout":8,"openMwTables":1,"latency":-4712,"videoProtocol":"flvjs","btTableView":"slingshot","btVideoQuality":"vip1_ss_hi","btMiniGame":0,"appVersion":4,"orientation":"landscape"}},"id":"29fm3e1f7m","type":"roulette.betAction"}))
                #wss.send(json.dumps(
                result =  wss.recv()
                data1 = json.loads(result)
                if data1["type"] == "roulette.betActionResponse":
                    self.TTa = True
            elif self.x == "f1f4rm9xgh4j3u2z" : #라 팔타지 자동 룰렛
                wss.send(json.dumps({"log":{"type":"CLIENT_BET_CHIP","value":{"amount":160,"type":"french_bet_black","chips":"160@french_bet_black","codes":{"ROU_Black50ReturnOn0":160},"balance":9480,"viewType":"Slingshot","gameType":"roulette","appVersion":4,"maxTouchPoints":0,"currency":"KRW","chipStack":[160,1000,4000,20000,100000,400000],"defaultChip":1,"tableMinLimit":160,"tableMaxLimit":2000000,"channel":"PCMac","orientation":"landscape"}}}))
                wss.send(json.dumps({"args":{"gameId":"15e344bc3e9152138cf16f9c","action":{"type":"PLACE","value":{"162":160}},"timestamp":self.tt(),"betTags":{"mwLayout":8,"openMwTables":1,"latency":-4707,"videoProtocol":"flvjs","btTableView":"slingshot","btVideoQuality":"spa_ss_hi","btMiniGame":0,"appVersion":4,"orientation":"landscape"}},"id":"s0pthxzjtw","type":"roulette.betAction"}))
                #wss.send(json.dumps(
                result =  wss.recv()
                data1 = json.loads(result)
                if data1["type"] == "roulette.betActionResponse":
                    self.TTa = True
            elif self.x == "q0a470up68p2b81p" : #드라고나 룰렛
                wss.send(json.dumps({"log":{"type":"CLIENT_BET_CHIP","value":{"amount":1000,"type":"bet_black","chips":"1000@bet_black","codes":{"ROU_Black":1000},"balance":8640,"viewType":"Immersive (Overlay, Full screen, Multi-camera View)","gameType":"roulette","appVersion":4,"maxTouchPoints":0,"currency":"KRW","chipStack":[1000,2000,10000,20000,1000000,2000000],"defaultChip":1,"tableMinLimit":1000,"tableMaxLimit":3000000,"channel":"PCMac","orientation":"landscape"}}}))
                wss.send(json.dumps({"args":{"gameId":"15e344dc9a5c92fa26e7e9e1","action":{"type":"PLACE","value":{"49":1000}},"timestamp":self.tt(),"betTags":{"mwLayout":8,"openMwTables":1,"latency":283,"videoProtocol":"flvjs","btTableView":"hd1","btVideoQuality":"drago_imr_hi","btMiniGame":0,"appVersion":4,"orientation":"landscape"}},"id":"kbb89ho2kh","type":"roulette.betAction"}))
                wss.send(json.dumps({"log":{"type":"CLIENT_EXITED_RFB_EDIT_MODE","value":{"channel":"PCMac","orientation":"landscape"}}}))
                result =  wss.recv()
                data1 = json.loads(result)
                if data1["type"] == "roulette.betActionResponse":
                    self.TTa = True
            elif self.x == "lkcbrbdckjxajdol" : #스피드 룰렛
                wss.send(json.dumps({"log":{"type":"CLIENT_BET_CHIP","value":{"amount":1000,"type":"bet_black","chips":"1000@bet_black","codes":{"ROU_Black":1000},"balance":8640,"viewType":"Immersive (Overlay, Full screen, Multi-camera View)","gameType":"roulette","appVersion":4,"maxTouchPoints":0,"currency":"KRW","chipStack":[500,1000,2000,10000,20000,1000000],"defaultChip":2,"tableMinLimit":500,"tableMaxLimit":3000000,"channel":"PCMac","orientation":"landscape"}}}))
                wss.send(json.dumps({"args":{"gameId":"15e344f2586c1453866f2d1a","action":{"type":"PLACE","value":{"49":1000}},"timestamp":self.tt(),"betTags":{"mwLayout":8,"openMwTables":1,"latency":269,"videoProtocol":"flvjs","btTableView":"hd1","btVideoQuality":"speed1_imr_hi","btMiniGame":0,"appVersion":4,"orientation":"landscape"}},"id":"8y3znd7czd","type":"roulette.betAction"}))
                wss.send(json.dumps({"log":{"type":"CLIENT_EXITED_RFB_EDIT_MODE","value":{"channel":"PCMac","orientation":"landscape"}}}))
                result =  wss.recv()
                data1 = json.loads(result)
                if data1["type"] == "roulette.betActionResponse":
                    self.TTa = True
            elif self.x == "lr6t4k3lcd4qgyrk" : #그랜드 카지노 룰렛
                wss.send(json.dumps({"log":{"type":"CLIENT_BET_CHIP","value":{"amount":1000,"type":"bet_black","chips":"1000@bet_black","codes":{"ROU_Black":1000},"balance":8640,"viewType":"Immersive (Overlay, Full screen, Multi-camera View)","gameType":"roulette","appVersion":4,"maxTouchPoints":0,"currency":"KRW","chipStack":[1000,2000,10000,20000,1000000,2000000],"defaultChip":1,"tableMinLimit":1000,"tableMaxLimit":3000000,"channel":"PCMac","orientation":"landscape"}}}))
                wss.send(json.dumps({"args":{"gameId":"15e345019643b0eb100590c0","action":{"type":"PLACE","value":{"49":1000}},"timestamp":self.tt(),"betTags":{"mwLayout":8,"openMwTables":1,"latency":295,"videoProtocol":"flvjs","btTableView":"hd1","btVideoQuality":"gcro1_imr_med","btMiniGame":0,"appVersion":4,"orientation":"landscape"}},"id":"9y54v7ple1","type":"roulette.betAction"}))
                wss.send(json.dumps({"log":{"type":"CLIENT_EXITED_RFB_EDIT_MODE","value":{"channel":"PCMac","orientation":"landscape"}}}))
                result =  wss.recv()
                data1 = json.loads(result)
                if data1["type"] == "roulette.betActionResponse":
                    self.TTa = True
            elif self.x == "AmericanTable001":#아메리칸 룰렛
                wss.send(json.dumps({"log":{"type":"CLIENT_BET_CHIP","value":{"amount":1000,"type":"bet_black","chips":"1000@bet_black","codes":{"RoDZ_Black":1000},"balance":8640,"viewType":"Immersive (Overlay, Full screen, Multi-camera View)","gameType":"americanroulette","appVersion":4,"maxTouchPoints":0,"currency":"KRW","chipStack":[200,1000,2000,10000,20000,1000000],"defaultChip":2,"tableMinLimit":200,"tableMaxLimit":3000000,"channel":"PCMac","orientation":"landscape"}}}))
                wss.send(json.dumps({"args":{"gameId":"15e3451e592aa7f5685b9898","action":{"type":"PLACE","value":{"408":1000}},"timestamp":self.tt(),"betTags":{"mwLayout":8,"openMwTables":1,"latency":273,"videoProtocol":"flvjs","btTableView":"hd1","btVideoQuality":"dzerot1_imr_hi","btMiniGame":0,"appVersion":4,"orientation":"landscape"}},"id":"onaacgfgmo","type":"roulette.betAction"}))
                wss.send(json.dumps({"log":{"type":"CLIENT_EXITED_RFB_EDIT_MODE","value":{"channel":"PCMac","orientation":"landscape"}}}))
                result =  wss.recv()
                data1 = json.loads(result)
                if data1["type"] == "roulette.betActionResponse":
                    self.TTa = True
            elif self.x == "mvrcophqscoqosd6": #카지노 몰타 룰렛
                wss.send(json.dumps({"log":{"type":"CLIENT_BET_CHIP","value":{"amount":1000,"type":"bet_black","chips":"1000@bet_black","codes":{"ROU_Black":1000},"balance":8640,"viewType":"Immersive (Overlay, Full screen, Multi-camera View)","gameType":"roulette","appVersion":4,"maxTouchPoints":0,"currency":"KRW","chipStack":[1000,2000,10000,20000,1000000,2000000],"defaultChip":1,"tableMinLimit":1000,"tableMaxLimit":3000000,"channel":"PCMac","orientation":"landscape"}}}))
                wss.send(json.dumps({"args":{"gameId":"15e34551cc99e2114d2c8e22","action":{"type":"PLACE","value":{"49":1000}},"timestamp":self.tt(),"betTags":{"mwLayout":8,"openMwTables":1,"latency":273,"videoProtocol":"flvjs","btTableView":"hd1","btVideoQuality":"casmlt1_imr_hi","btMiniGame":0,"appVersion":4,"orientation":"landscape"}},"id":"p0sejm6yia","type":"roulette.betAction"}))
                wss.send(json.dumps({"log":{"type":"CLIENT_EXITED_RFB_EDIT_MODE","value":{"channel":"PCMac","orientation":"landscape"}}}))
                result =  wss.recv()
                data1 = json.loads(result)
                if data1["type"] == "roulette.betActionResponse":
                    self.TTa = True
            elif self.x == "m2ghpkmua5jarg4t": #Resorts Atlantic City
                wss.send(json.dumps({"log":{"type":"CLIENT_BET_CHIP","value":{"amount":1000,"type":"bet_black","chips":"1000@bet_black","codes":{"RoDZ_Black":1000},"balance":8640,"viewType":"Immersive (Overlay, Full screen, Multi-camera View)","gameType":"americanroulette","appVersion":4,"maxTouchPoints":0,"currency":"KRW","chipStack":[1000,2000,10000,20000,1000000,2000000],"defaultChip":1,"tableMinLimit":1000,"tableMaxLimit":3000000,"channel":"PCMac","orientation":"landscape"}}}))
                wss.send(json.dumps({"args":{"gameId":"15e3458aebd3706477e3885a","action":{"type":"PLACE","value":{"408":1000}},"timestamp":self.tt(),"betTags":{"mwLayout":8,"openMwTables":1,"latency":313,"videoProtocol":"flvjs","btTableView":"hd1","btVideoQuality":"rcac1_imr_hi","btMiniGame":0,"appVersion":4,"orientation":"landscape"}},"id":"66s9vlkez1","type":"roulette.betAction"}))
                wss.send(json.dumps({"log":{"type":"CLIENT_EXITED_RFB_EDIT_MODE","value":{"channel":"PCMac","orientation":"landscape"}}}))
                result =  wss.recv()
                data1 = json.loads(result)
                if data1["type"] == "roulette.betActionResponse":
                    self.TTa = True
    def Countnumber(self, A) :
        if A == 0:
            self.Rads +=1
            self.Blacks +=1
            self.Nber1s +=1
    ...
    def Wsoc(self) :#7x0b1tgh7agmf6hv
        ...
        for op in range(len(dataa['args']['recentResults'])):
            self.nm2.append(int(dataa['args']['recentResults'][op][0]))
            self.nm2.reverse()
            print(self.nm2)
            for Cmn2 in range(len(nm2)):
                Cmns2 = int(self.nm2[Cmn2])
                self.Countnumber(Cmns2)
                if self.Nber1s == 4 : 
                    Beting(even,1000,even,ROU_Even,47,1000):#다운ex : from1to18, 1000, from1to18, ROU_118, 46, 1000
                    print ('짝수 배팅','홀수:',self.Nber1s, '짝수:',self.Nber2s, '레드:',self.Rads, '블랙:', self.Blacks, '업:',self.ups, '다운:',self.douns)
                elif self.Nber1s > 4 :
                    self.Bet1 *=2
                    Muni1 = self.Bet1*1000
                    Beting(even,Muni1,even,ROU_Even,47,Muni1):#다운ex : from1to18, 1000, from1to18, ROU_118, 46, 1000
                    print ('짝수 2배 배팅','홀수:',self.Nber1s, '짝수:',self.Nber2s, '레드:',self.Rads, '블랙:', self.Blacks, '업:',self.ups, '다운:',self.douns)
                else:
                    pass
                if self.Rads == 4:
                    Beting(black,1000,black,ROU_Black,49,1000):#다운ex : from1to18, 1000, from1to18, ROU_118, 46, 1000
                    print('블랙 배팅','홀수:',self.Nber1s, '짝수:',self.Nber2s, '레드:',self.Rads, '블랙:', self.Blacks, '업:',self.ups, '다운:',self.douns)
                elif self.Rads > 4:
                    self.Bet2 *=2
                    Muni2 = self.Bet2*1000
                    Beting(black,Muni2,black,ROU_Black,49,Muni2):#다운ex : from1to18, 1000, from1to18, ROU_118, 46, 1000
                    print('블랙 2배 배팅','홀수:',Nber1s, '짝수:',Nber2s, '레드:',Rads, '블랙:', Blacks, '업:',ups, '다운:',douns)
                else:
                    pass
                if self.ups == 4:
                    Beting(from1to18,1000,from1to18,ROU_118,46,1000):#다운ex : from1to18, 1000, from1to18, ROU_118, 46, 1000  
                    print('다운 배팅','홀수:',self.Nber1s, '짝수:',self.Nber2s, '레드:',self.Rads, '블랙:', self.Blacks, '업:',self.ups, '다운:',self.douns)
                elif self.ups > 4:
                    self.Bet3 *=2
                    Muni3 = self.Bet3*1000
                    Beting(from1to18,Muni3,from1to18,ROU_118,46,Muni3):#다운ex : from1to18, 1000, from1to18, ROU_118, 46, 1000    
