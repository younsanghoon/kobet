...
class Loby :
    ...
    def Lroom (self,y,ws,ROMS,NOBER,Money,Countnumber) : #처음 12개
        if self.nmaa1 == 0:
    ...
    def Countnumber(self) :
        if self.Cmns == 0:
            ...
            if self.Rad.count(self.Cmns):
                ...
                self.Blacks +=1
                self.Rads = 0
            if self.Cmns >= 1 and self.Cmns <= 18:
                self.douns +=1
                self.ups =0
                print('홀수:',self.Nber1s, '짝수:',self.Nber2s, '레드:',self.Rads, '블랙:', self.Blacks, '업:',self.ups, '다운:',self.douns)
            else:
                self.ups +=1
            return [self.ups,self.douns,self.Blacks,self.Rads,self.Nber1s,self.Nber2s]
                
    def betin(ws,y,ROMS,NOBER,Money):
        if self.Nber1s == 4 :
...
C8 = Count()#오토 룰렛
C9 = Count()#vip 오토 룰렛
C10 = Count()#라 팔타지 자동 룰렛
C11 = Count()#드라고나 룰렛
C13 = Count()#스피드 룰렛
C14 = Count()#그랜드 카지노 룰렛
C15 = Count()#아메리칸 룰렛
C16 = Count()#카지노 몰타 룰렛
C17 = Count()#Resorts Atlantic City
