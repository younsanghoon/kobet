    ...
    def Countnumber2(self) :
        if self.Cmns2 == 0:
            ...
            if self.Rad.count(self.Cmns2):
                ...
                self.Blacks +=1
                self.Rads = 0
            if self.Cmns2 >= 1 and self.Cmns2 <= 18:
                self.douns +=1
                self.ups =0
                #print('홀수:',self.Nber1s, '짝수:',self.Nber2s, '레드:',self.Rads, '블랙:', self.Blacks, '업:',self.ups, '다운:',self.douns)
            else:
...
with requests.session() as sess:
    ...
    ws.send(json.dumps({"id":wid(),"type":"metrics.ping","args":{"t":tt()}}))
KB1 = KoBet(ws,ROMS,NOBER,x,y,sessionId,wshead2)
KB2 = KoBet(ws,ROMS,NOBER,x,y,sessionId,wshead2)
KB3 = KoBet(ws,ROMS,NOBER,x,y,sessionId,wshead2)
KB4 = KoBet(ws,ROMS,NOBER,x,y,sessionId,wshead2)
KB5 = KoBet(ws,ROMS,NOBER,x,y,sessionId,wshead2)
KB8 = KoBet(ws,ROMS,NOBER,x,y,sessionId,wshead2)
KB9 = KoBet(ws,ROMS,NOBER,x,y,sessionId,wshead2)
KB10 = KoBet(ws,ROMS,NOBER,x,y,sessionId,wshead2)
KB11 = KoBet(ws,ROMS,NOBER,x,y,sessionId,wshead2)
KB13 = KoBet(ws,ROMS,NOBER,x,y,sessionId,wshead2)
KB14 = KoBet(ws,ROMS,NOBER,x,y,sessionId,wshead2)
KB15 = KoBet(ws,ROMS,NOBER,x,y,sessionId,wshead2)
KB16 = KoBet(ws,ROMS,NOBER,x,y,sessionId,wshead2)
KB17 = KoBet(ws,ROMS,NOBER,x,y,sessionId,wshead2)
while True:
    ws.send(json.dumps({"id":wid(),"type":"metrics.ping","args":{"t":tt()}}))
    for s in range(10):
        result =  ws.recv()
