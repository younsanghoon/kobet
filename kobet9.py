...
class Loby :
    ...
    def Lroom (self,y,ws,ROMS,NOBER,Money,Countnumber) :
...
class Lobys :
    def __init__ (self):
        self.nm = 0
    def Lrooms (self,y,nmaa1,Countnumber):#nmaa1 = Lroom.nmaa1
        ws.send(json.dumps({"id":wid(),"type":"metrics.ping","args":{"t":tt()}}))
        for s in range(10):
            result =  ws.recv()
            dataR = json.loads(result)
...
class Count:
    ...
    def Berting():
        if self.Nber1s == 4 : 
            BetingTime(even,1000,even,ROU_Even,47,1000):#다운ex : from1to18, 1000, from1to18, ROU_118, 46, 1000
            print ('짝수 배팅','홀수:',self.Nber1s, '짝수:',self.Nber2s, '레드:',self.Rads, '블랙:', self.Blacks, '업:',self.ups, '다운:',self.douns)
        elif self.Nber1s > 4 :
            self.Bet1 *=2
            Muni1 = self.Bet1*1000
            BetingTime(even,Muni1,even,ROU_Even,47,Muni1):#다운ex : from1to18, 1000, from1to18, ROU_118, 46, 1000
            print ('짝수 2배 배팅','홀수:',self.Nber1s, '짝수:',self.Nber2s, '레드:',self.Rads, '블랙:', self.Blacks, '업:',self.ups, '다운:',self.douns)
        else:
            pass
        if self.Rads == 4:
            BetingTime(black,1000,black,ROU_Black,49,1000):#다운ex : from1to18, 1000, from1to18, ROU_118, 46, 1000
            print('블랙 배팅','홀수:',self.Nber1s, '짝수:',self.Nber2s, '레드:',self.Rads, '블랙:', self.Blacks, '업:',self.ups, '다운:',self.douns)
        elif self.Rads > 4:
            self.Bet2 *=2
            Muni2 = self.Bet2*1000
            BetingTime(black,Muni2,black,ROU_Black,49,Muni2):#다운ex : from1to18, 1000, from1to18, ROU_118, 46, 1000
            print('블랙 2배 배팅','홀수:',Nber1s, '짝수:',Nber2s, '레드:',Rads, '블랙:', Blacks, '업:',ups, '다운:',douns)
        else:
            pass
        if self.ups == 4:
            Betingtime(from1to18,1000,from1to18,ROU_118,46,1000):#다운ex : from1to18, 1000, from1to18, ROU_118, 46, 1000  
            print('다운 배팅','홀수:',self.Nber1s, '짝수:',self.Nber2s, '레드:',self.Rads, '블랙:', self.Blacks, '업:',self.ups, '다운:',self.douns)
        elif self.ups > 4:
            self.Bet3 *=2
            Muni3 = self.Bet3*1000
            BetingTime(from1to18,Muni3,from1to18,ROU_118,46,Muni3):#다운ex : from1to18, 1000, from1to18, ROU_118, 46, 1000    
            print('다운 2배 배팅','홀수:',self.Nber1s, '짝수:',self.Nber2s, '레드:',self.Rads, '블랙:', self.Blacks, '업:',self.ups, '다운:',self.douns)
        else:
            pass
    
    
def RoomIn (self,ws,x,ROMS,NOBER,Money):#self.Money 현재 돈 ------------------------------------------------------------
    ...
    if ROM == "LightningTable01":
        ...
        Ws.send(json.dumps({"log":{"type":"CLIENT_LOBBY_TABLE_JOIN_CLICK","latency":La,"value":{"currency":"KRW","gameType":"lobby","buildId":"Build Version: 6.20191219.84546-f3434ac8 at 2019-12-19 09:02:37","device":"Desktop","typeApp":"lobby4","categoryId":"roulette","tablesCount":17,"layout":"Medium","filter":"NotAvailable","timeInLobby":11770,"tableId":"%s","tableName":"%s","tableOrder":NOBER,"channel":"PCMac","orientation":"landscape","balance":Money,"userAgent":"Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/79.0.3945.88 Safari/537.36","inGame":false}}}))%(x,ROMS)
...
def Wsoc(self,x) :#x = 7x0b1tgh7agmf6hv
    #룸 들어가기 함수
...
def BetingTime(self,A,B,C,D,E):#블랙ex : 
    self.TTa = False
