...
class KoBet :
    def __init__ (self,ws,ROMS,NOBER,x,y,sessionId):
    ...
    def Wsoc(self) :#x = 7x0b1tgh7agmf6hv
        ...
        self.CT = False
        self.Ws = create_connection("wss://livecasino.eg88play.com/public/roulette/player/game/%s/socket?messageFormat=json&instance=wrw3g-nx54o66tzywcx35y-%s&tableConfig=&EVOSESSIONID="+self.sessionId, headers = pharseHeaders(wshead2))%(self.x,self.x)
        wss = self.Ws
    ...
    if r1.status_code == 200:
        ...
        if r2.status_code == 200:
            ...
            if r3.status_code == 200:
                ...
                KB0 = KoBet(ws,"라이트닝 룰렛",1,x,"LightningTable01",sessionId)
                KB1 = KoBet(ws,"이머지브 룰렛",1,x,"7x0b1tgh7agmf6hv",sessionId)
                KB2 = KoBet(ws,"룰렛",2,x,"vctlz20yfnmp1ylr",sessionId)
                KB3 = KoBet(ws,"터키 룰렛",3,x,"r5aw9yumyaxgnd90",sessionId)
                KB4 = KoBet(ws,"룰렛 라이브",4,x,"zosmk25g2f768o52",sessionId)
                KB7 = KoBet(ws,"오토 룰렛",7,x,"48z5pjps3ntvqc1b",sessionId)
                KB8 = KoBet(ws,"VIP 오토 룰렛",8,x,"01rb77cq1gtenhmo",sessionId)
                KB9 = KoBet(ws,"라 팔타지 자동 룰렛",9,x,"f1f4rm9xgh4j3u2z",sessionId)
                KB10 = KoBet(ws,"드라고나 룰렛",10,x,"q0a470up68p2b81p",sessionId)
                KB12 = KoBet(ws,"스피드 룰렛",12,x,"lkcbrbdckjxajdol",sessionId)
                KB13 = KoBet(ws,"그랜드 카지노 룰렛",13,x,"lr6t4k3lcd4qgyrk",sessionId)
                KB14 = KoBet(ws,"아메리칸 룰렛",14,x,"AmericanTable001",sessionId)
                KB15 = KoBet(ws,"카지노 몰타 룰렛",15,x,"mvrcophqscoqosd6",sessionId)
                KB16 = KoBet(ws,"Resorts Atlantic City",16,x,"m2ghpkmua5jarg4t",sessionId)
